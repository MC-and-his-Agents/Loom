...
| EXT-0039 | `superpowers/README.md`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md`, `superpowers/.codex/INSTALL.md` | `superpowers` | `artifact` | 入口层 `skills` 若要作为上游能力复用，需要显式的安装、发现、升级合同，而不是把分发细节留在口头经验里。 | `superpowers` 为 Codex、OpenCode、Claude marketplace 等宿主分别提供安装、发现与更新路径，证明入口层分发本身需要正式合同。 | `configurable` | `skills` | `adapt` | 值得吸收为 Loom 的候选入口合同，但必须去除 symlink、plugin marketplace、特定目录等宿主实现细节。 |
| EXT-0040 | `superpowers/hooks/session-start`, `superpowers/skills/using-superpowers/SKILL.md`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md` | `superpowers` | `mechanism` | `bootstrap/root contract` 应保持轻薄，只负责把 agent 导向后续 skills 与引用关系，不应复制内核真相。 | `superpowers` 通过 session hook 注入 `using-superpowers`，再由后续 skill 继续分流，显示 root 入口负责导向与约束，而深知识保留在具体 skill 与引用材料中。 | `general` | `skills` | `adapt` | 适合作为 Loom `skills` 分发与适配合同的候选机制，但仍需更多宿主与仓库验证。 |
...
| EXT-0042 | `superpowers/hooks/hooks.json`, `superpowers/hooks/hooks-cursor.json`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md` | `superpowers` | `lesson` | 宿主特定的命令、路径、权限与插件细节应收敛在 adapter 层，而不应反向污染 `skills` 内核合同。 | `superpowers` 将 hook、安装路径、工具映射和平台差异分别放在宿主文档与 hook 适配层中，说明单一宿主实现细节可以被隔离，而不必写进 skills 核心。 | `general` | `skills` | `adapt` | 该结论与 Loom 宪法方向一致，但目前主要由 `superpowers` 单仓提供正向例证，仍需后续多宿主样本补强。 |
