...
| EXT-0022 | `Syvert/agent-loop.md`, `Syvert/code_review.md`, `WebEnvoy/AGENTS.md`, `WebEnvoy/code_review.md` | `both` | `lesson` | 长任务生命周期至少需要 `admission checkpoint`、`build checkpoint`、`merge checkpoint` 三类正式判断点。 | 两仓都已有 admission/spec review 与 merge gate，但中途 build 纠偏点偏弱，导致 merge 前 checkpoint 容易过载。 | `general` | `harness` | `keep` | Loom 现已由 `adoption/routing-and-checkpoints.md` 固化三类 checkpoint 的治理语义，并由 `harness/execution-chain.md` 与 `harness/merge-checkpoint.md` 承接执行侧链路与放行。 |
...
| EXT-0043 | `Loom GitHub issue practice`, `GitHub parent issue / sub-issue relation` | `loom` | `mechanism` | 跨多轮或阶段性推进的大事项，通常需要显式表达“父事项收敛目标与关闭语义，子事项承接可独立推进的执行单元”这一层级关系。 | 在 Loom 自身的 phase issue 与子 issue 收敛过程中，GitHub 的 parent issue / sub-issue 关系能够稳定表达阶段承接与执行拆分，但其字段与行为仍属于平台实现。 | `configurable` | `adoption` | `needs_validation` | Loom 应继续验证“父事项 / 子事项关系”这一抽象能力是否普遍需要，并在更多宿主与仓库中观察其关闭联动、状态汇总与可见性边界，再决定是否上移为更抽象的治理或 harness 能力。 |
| EXT-0044 | `loom-adoption-new-project`, `adoption/validation-new-project.md` | `loom-adoption-new-project` | `lesson` | 对空仓新项目，首轮 adoption 的目标应是建立根入口、最小治理、首批事项与升级入口，而不是预装重 harness。 | 新项目样本只有 `.git/` 且没有 `HEAD`、远端、规则入口和宿主承接面；这证明它首先缺的是进入 Loom 的最小结构，而不是完整恢复和状态支撑。 | `general` | `adoption` | `adapt` | 这是首个真实新项目样本，已足够进入候选落点，但仍需更多样本确认输出形状。 |
| EXT-0045 | `mail-listener`, `DevSkills`, `adoption/validation-devskills.md` | `both` | `lesson` | 轻量 retrofit 不仅要求已有边界文档，还要求已有仓库级验证入口；对共享 contract、shared skill 或 governance module 仓库，不应继续套用默认轻量路径。 | `mail-listener` 证明在已有 CI、测试和验证入口的前提下，轻量 retrofit 可以成立；`DevSkills` 则证明缺少统一验证入口且主产物本身是共享治理/skill 包时，这条路径会失效。 | `general` | `adoption` | `keep` | Loom 已据此收紧 `lightweight retrofit default` 与 `loom-init` 的轻量判定边界。 |
