...
| EXT-0039 | `superpowers/README.md`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md`, `superpowers/.codex/INSTALL.md` | `superpowers` | `artifact` | 入口层 `skills` 若要作为上游能力复用，需要显式的安装、发现、升级合同，而不是把分发细节留在口头经验里。 | `superpowers` 为 Codex、OpenCode、Claude marketplace 等宿主分别提供安装、发现与更新路径，证明入口层分发本身需要正式合同。 | `configurable` | `skills` | `keep` | Loom 已将其收敛为稳定入口合同，但仍明确把宿主特定安装路径、市场流程与目录实现留在 adapter 层。 |
| EXT-0040 | `superpowers/hooks/session-start`, `superpowers/skills/using-superpowers/SKILL.md`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md` | `superpowers` | `mechanism` | `bootstrap/root contract` 应保持轻薄，只负责把 agent 导向后续 skills 与引用关系，不应复制内核真相。 | `superpowers` 通过 session hook 注入 `using-superpowers`，再由后续 skill 继续分流，显示 root 入口负责导向与约束，而深知识保留在具体 skill 与引用材料中。 | `general` | `skills` | `keep` | Loom 已把这条结论收敛为稳定入口合同的一部分，并要求深知识仍落在被引用的规则主落点。 |
...
| EXT-0046 | `hotcp`, `adoption/validation-hotcp.md` | `hotcp` | `lesson` | 复杂既有仓库一旦同时存在共享契约、高风险边界、多运行面与多状态入口，就应从第一轮 adoption 起进入更完整装配，而不是继续停留在 `checkpoint-lite`。 | `hotcp` 同时具备控制面 / 数据面分离、多应用、多运行入口、严格数据隔离和正式部署 / 回滚链路，说明 issue / PR 文本不足以承接恢复与放行。 | `general` | `adoption` | `keep` | 这为 Loom 既有的复杂路径判断提供了真实复杂仓库样本补强。 |
