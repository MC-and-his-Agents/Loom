...
| EXT-0044 | `loom-adoption-new-project`, `new-project 主路径验证归档证据（Issue #168）` | `loom-adoption-new-project` | `lesson` | 对空仓新项目，首轮 adoption 的目标应是建立根入口、最小治理、首批事项与升级入口，而不是预装重 harness。 | 新项目样本只有 `.git/` 且没有 `HEAD`、远端、规则入口和宿主承接面；归档的新项目主路径证据进一步证明，它首先缺的是进入 Loom 的最小结构，而不是完整恢复和状态支撑。 | `general` | `adoption` | `adapt` | 这是首个真实新项目样本，已足够进入候选落点，但仍需更多样本确认输出形状。 |
...
| EXT-0046 | `hotcp`, `complex-existing 验证归档证据（hotcp）` | `hotcp` | `lesson` | 复杂既有仓库一旦同时存在共享契约、高风险边界、多运行面与多状态入口，就应从第一轮 adoption 起进入更完整装配，而不是继续停留在 `checkpoint-lite`。 | `hotcp` 同时具备控制面 / 数据面分离、多应用、多运行入口、严格数据隔离和正式部署 / 回滚链路，说明 issue / PR 文本不足以承接恢复与放行。 | `general` | `adoption` | `adapt` | 这条复杂路径升级信号目前主要来自 `hotcp` 单仓样本，先保留在候选落点；待第二个复杂仓库样本验证后再提升。 |
| EXT-0047 | `hotcp`, `complex-existing 验证归档证据（hotcp）` | `hotcp` | `lesson` | 既有复杂仓库若同时存在“现行规则入口”和“历史脚本 / 历史文档入口”，这本身就是恢复与状态升级信号。 | `hotcp` 的现行规则入口与历史脚本、历史工作流文档并存，恢复时若没有正式恢复主入口与状态面，就无法稳定判断应消费哪条真相。 | `configurable` | `adoption` | `adapt` | 目前主要由单仓样本支持，先进入候选落点，等待更多复杂仓库验证。 |
