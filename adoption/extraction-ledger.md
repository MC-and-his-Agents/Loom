...
| EXT-0039 | `superpowers/README.md`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md`, `superpowers/.codex/INSTALL.md` | `superpowers` | `artifact` | 入口层 `skills` 若要作为上游能力复用，需要显式的安装、发现、升级合同，而不是把分发细节留在口头经验里。 | `superpowers` 为 Codex、OpenCode、Claude marketplace 等宿主分别提供安装、发现与更新路径，证明入口层分发本身需要正式合同。 | `configurable` | `skills` | `adapt` | 目前主要由 `superpowers` 单仓样本支持，先保留在候选落点；待第二个独立样本验证后再提升为 `keep`。 |
| EXT-0040 | `superpowers/hooks/session-start`, `superpowers/skills/using-superpowers/SKILL.md`, `superpowers/docs/README.codex.md`, `superpowers/docs/README.opencode.md` | `superpowers` | `mechanism` | `bootstrap/root contract` 应保持轻薄，只负责把 agent 导向后续 skills 与引用关系，不应复制内核真相。 | `superpowers` 通过 session hook 注入 `using-superpowers`，再由后续 skill 继续分流，显示 root 入口负责导向与约束，而深知识保留在具体 skill 与引用材料中。 | `general` | `skills` | `adapt` | 目前主要由 `superpowers` 单仓样本支持，先保留在候选落点；待第二个独立样本验证后再决定是否进入稳定入口合同。 |
| EXT-0041 | `superpowers/tests/skill-triggering/**`, `superpowers/tests/explicit-skill-requests/**`, `superpowers/tests/claude-code/test-subagent-driven-development.sh`, `superpowers/tests/claude-code/test-subagent-driven-development-integration.sh` | `superpowers` | `mechanism` | 入口层能力需要覆盖“触发是否正确”和“行为是否退化”的回归测试，而不只验证代码产物。 | `superpowers` 为显式 skill 请求、隐式 skill 触发、多轮行为和子 agent 工作流建立了专门测试，说明入口层行为本身应进入自动化回归。 | `configurable` | `harness` | `adapt` | Loom 当前已把这类验证的候选落点收在 `harness/automation-frontload.md` 与 `skills/distribution-and-adapter-contract.md`，但是否进入默认 core 仍待更多宿主验证。 |
...
| EXT-0043 | `Loom GitHub issue practice`, `GitHub parent issue / sub-issue relation`, `adoption/validation-retrofit-143-tree.md` | `loom` | `mechanism` | 跨多轮或阶段性推进的大事项，通常需要显式表达“父事项收敛目标与关闭语义，子事项承接可独立推进的执行单元”这一层级关系。 | 在 Loom 自身的 phase issue 与子 issue 收敛过程中，GitHub 的 parent issue / sub-issue 关系能够稳定表达阶段承接与执行拆分；`#180` 又证明 Loom 自身真实树可以消费这套父子收口逻辑，但其字段与行为仍属于平台实现。 | `configurable` | `adoption` | `needs_validation` | `#180` 已补 Loom 自身的 live retrofit 证据，但这仍是单宿主、单仓库样本；在更多宿主与仓库完成第二样本前，继续维持 `needs_validation`，不直接上移为更抽象的治理或 harness 能力。 |
| EXT-0044 | `loom-adoption-new-project`, `adoption/validation-new-project.md`, `adoption/validation-main-path-new-project.md` | `loom-adoption-new-project` | `lesson` | 对空仓新项目，首轮 adoption 的目标应是建立根入口、最小治理、首批事项与升级入口，而不是预装重 harness。 | 新项目样本只有 `.git/` 且没有 `HEAD`、远端、规则入口和宿主承接面；`#168` 的主路径验证进一步证明它首先缺的是进入 Loom 的最小结构，而不是完整恢复和状态支撑。 | `general` | `adoption` | `adapt` | 这是首个真实新项目样本，已足够进入候选落点，但仍需更多样本确认输出形状。 |
| EXT-0045 | `mail-listener`, `DevSkills`, `adoption/validation-devskills.md`, `adoption/validation-existing-repo-execution-sync.md` | `both` | `lesson` | 轻量 retrofit 不仅要求已有边界文档，还要求已有仓库级验证入口；对共享 contract、shared skill 或 governance module 仓库，不应继续套用默认轻量路径。 | `mail-listener` 证明在已有 CI、测试和验证入口的前提下，轻量 retrofit 可以成立；`DevSkills` 则证明缺少统一验证入口且主产物本身是共享治理/skill 包时，这条路径会失效；`#170` 进一步补上既有仓库执行/回写与 sync repo companion 的正式验证记录。 | `general` | `adoption` | `keep` | Loom 已据此收紧 `lightweight retrofit default` 与 `loom-init` 的轻量判定边界。 |
| EXT-0046 | `hotcp`, `adoption/validation-hotcp.md` | `hotcp` | `lesson` | 复杂既有仓库一旦同时存在共享契约、高风险边界、多运行面与多状态入口，就应从第一轮 adoption 起进入更完整装配，而不是继续停留在 `checkpoint-lite`。 | `hotcp` 同时具备控制面 / 数据面分离、多应用、多运行入口、严格数据隔离和正式部署 / 回滚链路，说明 issue / PR 文本不足以承接恢复与放行。 | `general` | `adoption` | `adapt` | 这条复杂路径升级信号目前主要来自 `hotcp` 单仓样本，先保留在候选落点；待第二个复杂仓库样本验证后再提升。 |
| EXT-0047 | `hotcp`, `adoption/validation-hotcp.md` | `hotcp` | `lesson` | 既有复杂仓库若同时存在“现行规则入口”和“历史脚本 / 历史文档入口”，这本身就是恢复与状态升级信号。 | `hotcp` 的现行规则入口与历史脚本、历史工作流文档并存，恢复时若没有正式恢复主入口与状态面，就无法稳定判断应消费哪条真相。 | `configurable` | `adoption` | `adapt` | 目前主要由单仓样本支持，先进入候选落点，等待更多复杂仓库验证。 |
| EXT-0048 | `Syvert`, `WebEnvoy`, `adoption/validation-deep-existing-repo-syvert-webenvoy.md` | `both` | `lesson` | 成熟治理重仓若已有清晰根规则、统一验证入口并出现 merge-review overload，应继续保持 `complex-existing`，但默认走 attach-only 的 `deep-existing-repo`，而不是重写成 `full-bootstrap` 或新增第四种 `repository_mode`。 | `Syvert` 与 `WebEnvoy` 都要求 Loom 先接入入口与读面，而不是重写其 root rules、retained host actions 与 repo-native carriers；`#247` 进一步证明，在显式成熟治理信号成立时，`loom-init` 应返回 `recommended_adoption.path = deep-existing-repo` 且保持 attach-only。 | `general` | `adoption` | `keep` | 这条路径已成为 Loom 的稳定 adoption 合同，但仍只是 `complex-existing` 下的子路径。 |
| EXT-0049 | `reference-companion-spec-syvert.md`, `reference-companion-spec-webenvoy.md`, `adoption/validation-deep-existing-repo-syvert-webenvoy.md` | `both` | `mechanism` | `repo companion` 需要 typed `specialized_gates` 与 locator-first `context_schema` 的机读合同，但不应吸收 repo-specific 语义细节。 | `Syvert` 与 `WebEnvoy` 都要求 Loom 能区分 gate 所属 surface，并声明 repo-specific context fields；`#245/#247` 进一步证明 `repo-interface v2` 在保持 `v1` 可读的前提下足以承接这两类信息。 | `general` | `adoption` | `keep` | typed gates 与 `context_schema` 已进入稳定合同，repo-specific 字段命名仍留在 companion 层。 |
| EXT-0050 | `WebEnvoy/.github/PULL_REQUEST_TEMPLATE.md`, `reference-companion-spec-webenvoy.md`, `adoption/validation-deep-existing-repo-syvert-webenvoy.md` | `webenvoy` | `mechanism` | repo-specific metadata contract 可以作为 companion-owned 的可选机读节声明，但字段集合与适用条件不应提前固化成 Loom core 默认模型。 | `WebEnvoy` 需要 `integration_check`、`gate_applicability`、`live_evidence_record` 等 metadata block；当前样本足以证明 `metadata_contract` 这个扩展位有价值，但不足以证明统一字段 taxonomy 已稳定。 | `configurable` | `adoption` | `adapt` | `metadata_contract` 进入 `repo-interface v2` 作为可选扩展，字段 taxonomy 继续停在候选区。 |
| EXT-0051 | `Syvert`, `WebEnvoy`, `adoption/repo-interop-contract.md`, `adoption/validation-deep-existing-repo-syvert-webenvoy.md` | `both` | `mechanism` | retained host action result、repo-native carriers 与 shadow parity 需要单独的 interop 读面，而不是回塞到 `repo-interface.json` 或接管宿主实现。 | 两个样本都存在宿主动作结果与 repo-native truth；`#246/#247` 证明把这些入口收进独立 `interop.json`，可以同时保持宿主 ownership 与 Loom read-only consumption 的边界。 | `general` | `adoption` | `keep` | `interop.json` 已成为稳定合同，但 adapter payload 与 carrier shape 继续留在 repo-local 层。 |
| EXT-0052 | `Syvert`, `WebEnvoy`, `adoption/repo-interop-contract.md`, `adoption/validation-deep-existing-repo-syvert-webenvoy.md` | `both` | `lesson` | 在成熟治理重仓中，`shadow parity` 更适合作为 `admission/review/merge_ready/closeout` 的 validation-only compare surface，而不是直接改写 merge gate。 | 两个样本都要求 Loom 能对照 repo-native verdict，但当前证据只稳定支持 read-only compare；因此 `#247` 把 `shadow mode` 固定为 validation-only。 | `general` | `adoption` | `keep` | `shadow parity` 的 compare 面已进入稳定合同，但不会在本树内自动变成新的 merge gate。 |
| EXT-0053 | `Syvert`, `WebEnvoy`, `adoption/validation-deep-existing-repo-syvert-webenvoy.md` | `both` | `mechanism` | 把 parity mismatch 自动提升为 blocking merge gate，需要更多 live adopted repo 证据。 | 当前 `shadow mode` 只验证 compare 面，不承担“哪一方自动获胜”；`Syvert` / `WebEnvoy` 两个样本仍不足以证明 mismatch 应直接阻断 merge / closeout。 | `configurable` | `adoption` | `needs_validation` | 在获得更多 live adopted repo 证据前，继续停在待验证区。 |
