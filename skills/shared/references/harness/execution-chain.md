...
| 每轮读取 | `Work Item`、恢复主入口、工作现场、最近验证摘要 | 本轮单一推进单元、当前上下文基线 | [execution-context.md](./execution-context.md) | 退回恢复主入口或状态控制面补齐事实 |
...
| 验证汇总 | 自动检查结果、人工验证、运行证据入口 | 最近验证摘要、未决阻断项、是否可进 gate chain | [status-surface.md](./status-surface.md) | 回到验证入口继续补验证或声明 `not_applicable` |
...
| build gate | 最新事实链、事项范围、入口完整性、关键阻断项 | `pass`、`block` 或 `fallback` | `checkpoint-model.md`（build） | 回到 `Work Item` / recovery 回写补齐输入或收敛范围 |
| review gate | `flow review` 基线、build 结果、review evidence、review record | reviewer 结论可被 merge gate 消费 | [review-execution.md](./review-execution.md) | 回到 build / review run / recovery 回写 / review record |
| merge gate | build 结果、最新 head、验证摘要、风险与回滚、未决项、宿主 merge 控制面 | `允许放行`、`阻断待补` 或 `退回前序 gate` | [merge-checkpoint.md](./merge-checkpoint.md) | 按结果回退到 build / spec / 验证阶段继续补材料 |
