...
| `review` | `loom flow review --target <repo> [--item <id>]` -> `loom review run --target <repo> [--item <id>]` -> `loom review record --target <repo> [--item <id>] ...` | runtime-state + fact-chain + state-check + runtime evidence + build checkpoint + review record | `pass` / `block` / `fallback` | 正式 review 先读基线，再运行默认 reviewer 并显式记录 reviewer 结论；`runtime_state` 不一致时直接 fail-closed |
