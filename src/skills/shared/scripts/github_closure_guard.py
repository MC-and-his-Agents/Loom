...
def _payload(subject: int, verdict: str, summary: str, reasons: list[dict[str, str]], *, completed: bool) -> dict[str, Any]:
    ...
    if verdict == "reopen_required":
        ...
        delivery_failure = not acceptance_failure and any(
            reason.get("code") == "missing_delivery_attestation" for reason in reasons
        )
        cause = primary_cause(
            cause_id=(
                "fr_phase_product_acceptance_invalid"
                if acceptance_failure
                else "fr_phase_delivery_attestation_invalid"
                if delivery_failure
                else "fr_phase_closure_invalid"
            ),
            failure_domain="product_acceptance" if acceptance_failure else "governance_metadata",
            code=(
                "product_acceptance_invalid"
                if acceptance_failure
                else "missing_delivery_attestation"
                if delivery_failure
                else "closure_invalid"
            ),
