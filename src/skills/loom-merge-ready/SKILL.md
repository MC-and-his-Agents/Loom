...
普通交付默认使用 `loom ship`；本 skill 用于显式合并前预检、阻断诊断或需要单独
读取 merge gate 摘要的场景。

优先入口：

- `loom merge-ready --target <repo> [--item <id>] --json`
- 普通交付：`loom ship --target <repo> --item <id> --issue <n> --pr <n> --branch <branch> --head-sha <sha> --apply --json`
...
- 不把自己当作日常交付主路径；`loom ship` 会消费合并就绪、拉取请求门控、
  受控合并和收尾策略，并在策略允许时完成内联或仅宿主收尾
