...
| `scenario-wrappers-and-gates` | `route`, `status`, `fact-chain`, `profile`, `story/spec/plan/build/pre-review/handoff/retire/closeout`, `gate *`, `merge-ready` | CLI wrapper contracts, active item suite gate consumption, PR head drift fixture, gate-freeze command/help/readiness contract, failure-classifier mapping (`pr_metadata_drift`, `shadow_stale`, `unsupported_command_surface`, `hosted_snapshot_mismatch`), and hosted-admission snapshot mismatch coverage. Closeout gate consumption stays in scope here only as wrapper coverage, not closeout terminal behavior. |
