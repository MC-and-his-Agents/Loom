...
| EXT-0053 | `Syvert`, `WebEnvoy`, `deep-existing-repo 验证归档证据（Issue #247）`, `shadow parity 升级证据标准（Issue #277）` | `both` | `mechanism` | 把 parity mismatch 自动提升为 blocking merge gate，需要更严格的 live adopted repo 证据，而且 blocking ownership、override path 与 authority-of-truth 不得由 `interop.json` 越权承接。 | 当前 `shadow mode` 只验证 compare 面，不承担“哪一方自动获胜”；`Syvert` / `WebEnvoy` 的现有归档证据仍不足以证明 mismatch 应直接阻断 merge / closeout。`#277` 进一步把升级门槛收紧为：至少两个新增 live adopted repo、四个 surface 的版本化 parity 记录、稳定 mismatch 分型，以及可证明的 gate 收益。 | `configurable` | `adoption` | `needs_validation` | 证据标准已经进入版本控制，但在满足这些条件前，`shadow parity` 仍固定保持 validation-only。 |
| EXT-0054 | `Syvert/WORKFLOW.md`, `Syvert/scripts/governance_status.py`, `Syvert/scripts/item_context.py`, Loom Issues `#298-#309` | `syvert` | `mechanism` | Syvert 级治理能力应拆成 Loom core 控制面与 GitHub profile 自动化，而不是以 Syvert 文件名原样迁移。 | `#298-#309` 对照验证表明，通用部分是 `Work Item` 唯一入口、host binding、status control plane、failure taxonomy、gate chain 与 maturity model；GitHub 特定部分是 FR/PR/merge commit 绑定、controlled merge、closeout/reconciliation 与升级自动化。 | `general` | `harness` | `keep` | 当前已落到 `harness/host-issue-binding.md`、`harness/status-surface-contract.md`、`harness/gate-chain.md`、`harness/governance-failure-taxonomy.md`、`harness/controlled-merge.md` 与 `governance/governance-maturity-model.md`。 |
| EXT-0055 | `Syvert/spec_guard.py`, `Syvert/spec_review.md`, `Syvert/code_review.md`, Loom strong governance parity validation | `syvert` | `mechanism` | formal spec、implementation review、merge-ready 与 closeout 必须作为强前置消费链运行，后序成功不得覆盖前序缺失。 | Syvert 的 spec/review/merge 纪律证明了前序 gate 缺失会导致 merge review 过载；Loom 的 strong governance parity 归档将该经验收成 `spec gate -> implementation review -> merge-ready -> controlled merge -> closeout`。 | `general` | `harness` | `keep` | 当前已由 `harness/gate-chain.md`、`harness/merge-checkpoint.md`、`harness/closeout-gate.md` 与 `tools/loom_status.py` / `tools/loom_flow.py governance-profile` 承接。 |
| EXT-0056 | `Loom Issue #410`, `zero-friction-adoption-contract.md`, `loom-adopt`, `loom-resume` | `loom` | `mechanism` | agent-assisted adoption 应固定为 `read -> judge -> write -> verify` 的入口合同，消费 repo companion / repo interop 边界但不新增事实载体。 | `#410` 将 zero-friction adoption 收敛为可执行入口和输出合同，证明采用流程需要由 agent 可执行合同承接，而不是让用户先学习完整治理体系。 | `general` | `adoption` | `keep` | 当前已落到 `adoption/zero-friction-adoption-contract.md`、`skills/loom-adopt/SKILL.md`、`skills/loom-adopt/references/input-signals.md`、`skills/loom-adopt/references/output-contract.md`、`skills/loom-resume/SKILL.md` 与 `skills/loom-resume/references/output-contract.md`。 |
| EXT-0057 | `Loom Issue #439`, `superpowers/README.md`, `superpowers/skills/**`, `superpowers/hooks/**`, `superpowers/tests/**` | `superpowers` | `lesson` | Superpowers-derived execution discipline should be absorbed as operating-layer contracts, not copied as a parallel truth source, documentation tree, or skill/file layout. | `#439` upgrades Loom from a governance-shaped repository into an agent-first project operating layer while preserving plugin / SKILLS / CLI / docs / fixtures as the product shape; the Superpowers evidence supports install/discovery discipline, scenario entry behavior, trigger regression, subagent ownership, and adapter boundaries, but does not justify importing `docs/superpowers/*`, Superpowers hook files, or its skill text into Loom core. | `general` | `harness` | `keep` | The landing targets are the existing operating planes: `governance truth`, `harness orchestration`, `behavior and test evidence`, and `executable SKILLS`. `governance truth` remains one plane of the operating layer rather than the whole positioning; no `docs/superpowers/*` subtree is created. |
| EXT-0058 | `Loom Issue #439`, `Loom Issue #441`, `Loom Issue #442`, `Syvert/docs/specs/_template/spec.md`, `Syvert/docs/specs/_template/plan.md` | `loom` | `mechanism` | Work Item -> spec -> plan -> build checkpoint -> review -> merge-ready -> closeout should carry a dual evidence loop: BDD for observable behavior and TDD for implementation proof. | `#439` defines behavior-first execution as the productization phase, with BDD as the outer observable behavior contract and TDD as the inner implementation evidence loop; existing Syvert-derived spec and plan templates already contain GWT scenarios and TDD planning hooks, but #439 requires those signals to be consumable by gates, status, review records, and closeout rather than staying as template prose. | `general` | `harness` | `keep` | This does not make BDD/TDD a mandatory ritual for pure documentation changes. Landing should reuse existing `templates`, `harness`, `governance`, `skills`, and check surfaces instead of creating a separate behavior-evidence directory. |
| EXT-0059 | `Loom PR #762`, `Loom Issue #763`, `validation-pr-762-semantic-review-gate-gap.md` | `loom` | `mechanism` | Loom's semantic review hard gate must be host-enforced by a PR-specific check bound to the current PR head; broad repository checks are not enough. | PR `#762` merged while live readback showed no GitHub review records, no active rulesets, required approving review count `0`, and branch protection requiring only broad checks. That proved the internal gate model and host merge enforcement were out of sync. | `general` | `harness` | `keep` | Current landing is `harness/pr-merge-gate.md`, `harness/controlled-merge.md`, `harness/host-action-contract.md`, and generated `shared/references/harness/pr-merge-gate.md`; raw review/shadow evidence remains runtime evidence only, not approval truth. |
| EXT-0060 | `HotCP story / acceptance scenario flow`, `docs/methodology/governance/story-intake.md` | `hotcp` | `lesson` | Story intake needs a lightweight user-owned business semantic confirmation point before formal spec / plan consume story scenarios. | HotCP showed that Story / acceptance scenarios can be generated and mechanically or semantically reviewed by agents while the user never gets a stable, narrow point to confirm actor, capability, outcome, value, scenarios, and non-goals. That lets business scope drift enter spec / plan as if already accepted. | `general` | `governance` | `keep` | This does not import HotCP-specific workflow. It repairs Loom's existing story-intake authority boundary by adding Story Business Confirmation with `pending`, `confirmed`, `revision-requested`, and `not-applicable` outcomes. |
| EXT-0061 | `github/spec-kit` DeepWiki research, Loom Issue `#1013`, Loom Issues `#1014`-`#1016` | `spec-kit` | `principle` | Spec-driven development should be internalized as a Loom execution discipline, not treated as either external to Loom or as Loom's whole product boundary. | `github/spec-kit` demonstrates that staged specification, planning, task generation, implementation, and analysis can give agents a stronger execution path than one-shot coding. The `#1013` planning discussion showed this is compatible with Loom because Loom already owns Work Item, spec, plan, review, merge-ready, closeout, host binding, and evidence consumption; the key boundary is that SDD strengthens formal spec / implementation paths without replacing adoption, resume, handoff, review, merge-ready, closeout, repo companion, or host control surfaces. | `general` | `template` | `keep` | Landing stays in Loom's existing project operating layer docs and spec-suite boundary. This does not make Loom an SDD-only tool. |
| EXT-0062 | `github/spec-kit` DeepWiki research, Loom Issues `#1014`-`#1016` | `spec-kit` | `mechanism` | Spec-kit style staged artifacts and cross-artifact analysis should be adapted into Loom's full spec suite, delivery planning, execution breakdown, evidence-map, and consistency-analysis contracts. | Spec-kit's `spec -> plan -> tasks -> implement -> analyze` flow shows value in explicit upstream/downstream artifact consumption, template-constrained output, prerequisite checks, and consistency analysis. Loom should adapt those mechanisms into its own formal spec path while preserving Work Item as execution entry and host-backed gate consumption. | `configurable` | `template` | `adapt` | The keep-level boundary is only the discipline and direction. Full suite shape, task carrier, GitHub project mapping, and gate integration remain assigned to `#1014`-`#1016` and later FRs. |
| EXT-0063 | `github/spec-kit` DeepWiki research, Loom Issue `#1013` | `spec-kit` | `anti_pattern` | Spec-kit concrete product surfaces should not be copied into Loom core as default file layout, slash-command names, or trust model. | Spec-kit is a spec-driven development toolkit with `.specify/`, `/speckit.*` commands, fixed scaffolding conventions, preset/extension mechanisms, and a trust-based extension model. Loom is a project operating layer; copying those surfaces would violate Loom's rule against importing external source-specific documentation trees, file layouts, or host assumptions as core. | `repo_specific` | `template` | `drop` | Reject-as-is items may remain as historical evidence, but they do not get Loom core landing paths. Future work may re-evaluate individual mechanisms only after abstraction and safety boundaries are defined. |
