...
- Carrier: root `.loom/`
- Plugin install surface: repo-local Codex marketplace at `.agents/plugins/marketplace.json`, pointing to the Loom plugin root `./`
...
- The canonical skills source remains `skills/`; downstream installer payloads are generated under `plugins/loom` for target repos and must not be committed in Loom root.
