...
- Carrier: root `.loom/`
- Plugin install surface: repo-local Codex marketplace at `.agents/plugins/marketplace.json`, pointing to ignored generated payload `./plugins/loom`
...
- The canonical skills source remains `skills/`; the root repo-local plugin payload is generated by the installer and must not be committed.
