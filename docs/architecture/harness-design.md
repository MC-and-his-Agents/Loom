...
| work item / `exec-plan` | `../methodology/harness/work-item-contract.md` | 定义正式执行单元；本文只定义它在完整方案中的装配位置 |
| 单一事实链 | `../methodology/harness/fact-chain-contract.md` | 定义静态真相、动态真相与派生读面的归属关系 |
| 执行上下文 | `../methodology/harness/execution-context.md` | 定义上下文合同；本文只要求每轮正式执行必须绑定 |
| 最小执行链路 | `../methodology/harness/execution-chain.md` | 定义阶段顺序、主入口与回退去向；本文只说明它如何与初始化和强度模型组合 |
| 工作现场 | `../methodology/harness/workspace-model.md` | 定义隔离现场与恢复定位；本文只定义它与纯度和恢复的组合关系 |
| 纯度预检 | `../methodology/harness/workspace-and-purity.md` | 定义纯度规则；本文只定义它在正式执行前的作用 |
| 恢复模型 | `../methodology/harness/recovery-model.md` | 定义 `checkpoint`、`resume`、`handoff` 与回写事实；本文不重复恢复字段 |
| 状态面与运行证据 | `../methodology/harness/status-surface.md` | 定义状态读取字段、运行时证据入口与 `not_applicable` |
| 自动化前置 | `../methodology/harness/automation-frontload.md` | 定义检查矩阵与覆盖边界 |
| merge checkpoint | `../methodology/harness/merge-checkpoint.md` | 定义执行侧放行输入、结果语义与回退承接 |
...
- 每轮执行的阶段顺序与回退去向，以 [../methodology/harness/execution-chain.md](../methodology/../methodology/harness/execution-chain.md) 为准
- 状态读取、运行证据、自动化前置和 merge checkpoint 都应消费同一组执行真相，不制造并行真相源
- 字段 authored 权限与派生边界，以 [../methodology/harness/fact-chain-contract.md](../methodology/../methodology/harness/fact-chain-contract.md) 为准
- merge checkpoint 的执行语义以 [../methodology/harness/merge-checkpoint.md](../methodology/../methodology/harness/merge-checkpoint.md) 为准；本文不再重复定义输入集和结果枚举
