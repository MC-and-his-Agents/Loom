...
Loom 的目标不是只拥有一组文档，而是形成一套完整、可复用、可升级的项目运行系统。

目标状态至少包括：

- 完整治理方案
  - 事项入口、事项分流、规格准入、审查模型、成熟度与关闭语义清晰稳定
- 完整 harness 方案
  - 初始化、执行上下文、工作现场、恢复模型、状态面、运行时可见性、自动化前置清晰稳定
  - 不只满足最小机械能力，还要形成可被下游直接消费的完整执行内核
  - 高频执行动作、checkpoint 承接、工作现场治理与 gate 入口有稳定脚本或等价自动化入口
- 结构化模板体系
  - 正式规约模板、PR 模板和条件化模板块清晰稳定
- 可装配入口
  - `SKILLS` 能根据仓库场景选择并装配 Loom 能力
- 多仓验证闭环
...
1. `Phase A`
   定义完整治理方案与完整 harness 方案
2. `Phase B`
   把方案收敛为稳定能力组件与仓库结构
3. `Phase C`
   建立初始化、模板与装配入口
4. `Phase D`
   建立执行支撑、自动化前置与运行可见性能力
5. `Phase E`
   完成多场景验证、版本化演进与上游交付
...
这一阶段的目标，是先把 Loom 要成为怎样的系统用语言定义清楚。
...
- Loom 的治理方案和 harness 方案已经可以被完整描述
- 系统总图与详细设计的分工已经清楚
- 关键能力已经有明确落点，而不是停留在会话判断中
...
这一阶段的目标，是让 Loom 不只是“被阅读”，而是“能被接入”。

核心产出：

- 初始化入口
  - 能识别新项目、小型既有仓库、复杂既有仓库
- 装配逻辑
  - 能根据场景选择治理、harness、模板的组合
- 初始化输出合同
  - 首批工件
  - 首批事项
  - checkpoint 策略
...
- Loom 已经具备明确的初始化与装配入口
- 不同仓库场景下的默认接入方式已经能被稳定表达
- 下游仓库可以基于 Loom 开始第一轮接入，而不是只能人工理解整套文档

## 7. Phase D：建立执行支撑与机械能力

这一阶段的目标，是把 Loom 的 harness 从“设计上成立”推进到“形成完整执行内核”。
...
- `Phase A`
  - 先定义完整方案，避免后续能力建设失去目标
- `Phase B`
  - 再把方案落成稳定组件，避免所有规则混在少数总文档里
- `Phase C`
  - 再建立入口与装配能力，避免 Loom 只能被阅读，不能被接入
- `Phase D`
  - 再建立更强执行支撑与机械能力，避免 harness 只停留在概念上
- `Phase E`
