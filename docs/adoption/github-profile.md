...
jobs:
  gate:
    uses: MC-and-his-Agents/Loom/.github/workflows/loom-delivery-gate.yml@LOOM_40_CHAR_SHA
    with:
      validation_command: make py-compile
      enforcement: enforce
      profile: light
```

`validation_command` 必须由目标仓提供，不能省略、使用动态空值或只执行
`make delivery-gate-check` evaluator 自测。
