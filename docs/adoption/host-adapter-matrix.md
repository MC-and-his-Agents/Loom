...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Codex | primary | `npm install -g @mc-and-his-agents/loom`; `loom host install --host codex --scope user --apply --json`; `loom host register --host codex --scope user --apply --json`; downstream repository adoption uses `loom install --target . --apply --json` | metadata-only repositories consume the user-level Codex Loom plugin provider; no repo-local Loom plugin or skills payload is written | new Codex session or Codex Desktop restart after registration, then start from `loom-init` | Codex tools remain host-owned; Loom skills describe required actions | update root CLI, rerun user-level host install/register, then rerun installed-state validation | metadata-only: `loom host verify --host codex --target . --json`; workstation dry-run: `loom host install --host codex --scope user --dry-run --json` and `loom host register --host codex --scope user --dry-run --json`; `loom skills check --target . --json` |
