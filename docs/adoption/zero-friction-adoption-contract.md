...
当 `adoption_intent = attach-only` 时，dry-run 输出必须列出 required carriers 与 `forbidden_authored_carriers`，write 后只持久化 bootstrap manifest 和 repo companion locator。默认禁止 `.loom/work-items/**`、`.loom/progress/**`、`.loom/status/current.md`、`.loom/reviews/**`、`.loom/specs/**`、`.loom/shadow/**` 与运行态 `init-result` snapshot；verify 必须从 bootstrap manifest readback 并检查磁盘实际状态。发现任一 forbidden carrier 时必须 fail closed，并要求迁移到宿主 truth locator、删除 competing carrier，或显式升级到 `execution-control`。

当 `adoption_intent = light-governance` 时，dry-run / write 必须保持 lightweight retrofit 的最小治理边界：允许 `.loom/README.md`、只含 profile、repo-relative locator 与 capabilities 的 `.loom/bootstrap/manifest.json`、repo companion locator 与 PR 模板；不得生成、声明或保留 Loom-owned `.loom/work-items/**`、`.loom/progress/**`、`.loom/status/current.md`、`.loom/specs/**`、`.loom/reviews/**`、`.loom/shadow/**`、`.loom/bin/**`、`.loom/runtime/**`、`.loom/tmp/**` 或运行态 `init-result` snapshot。未显式选择 profile 时，新仓 `full-bootstrap` 建议也必须落到这一 host-derived 默认面；只有显式请求才可进入 `execution-control` / `strong-governance` 兼容面。默认 `.loom/` 文件数必须低于 10；如果目标仓库确实需要 repo-local execution carriers，必须显式升级，且普通 route/status/closeout 不得隐式回退该兼容面。
