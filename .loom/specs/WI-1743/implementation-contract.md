# WI-1743 Implementation Contract

## Ownership

- Owns root release authority: `VERSION` and root `package.json`.
- Owns v0.20.0 plugin payload release metadata in `plugins/loom/.codex-plugin/plugin.json`.
- Owns v0.20.0 release readiness evidence and WI-1743 carriers.
...
- Do not close #1734 or milestone #17 until #1743 release readback proves v0.20.0 publication and no stale child issue remains.
...
- If v0.20.0 is not published, revert the release PR before merge or bump to a new unoccupied version before retrying.
