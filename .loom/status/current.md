...
- Diagnostics Entry: gate freeze records review/head binding schema, decision, kind, reviewed/current/pr head, changed paths, disallowed paths, semantic_review_disposition consumption, binding_status, and next_action. Python 3.11 release fixture, PR gate carrier-only approval lint, and PR gate machine-carrier body binding are fixed locally; current PR #1528 metadata/readback and implementation review must be refreshed after the next push.
