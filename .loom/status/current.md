...
- Verification Entry: `git diff --check`; `PYTHONDONTWRITEBYTECODE=1 python3 tools/loom.py suite carrier validate --target . --item WI-1495 --json`; `PYTHONDONTWRITEBYTECODE=1 python3 .loom/bin/loom_flow.py fact-chain --target .`; `PYTHONDONTWRITEBYTECODE=1 python3 .loom/bin/loom_flow.py shadow-parity --target . --surface all --blocking`; `gh pr view 1663 --json number,state,mergedAt,mergeCommit,url`; `gh issue view 1495 --json number,state,closedAt,url`; `gh issue view 1496 --json number,state,closedAt,url`.
