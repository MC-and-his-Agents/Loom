...
`skills/` 是 Loom 的场景封装层。

当 Loom 以 repo-local `loom` plugin 的方式被安装到 Codex、Claude Code 或其他 agent 平台后，用户真正会看到并调用的，就是这里的 skill 名称。
...
不要把 `skills/` 当成新的事实真相源，也不要把裸 `skills/` 目录当成 Loom 的正式安装面。这里的职责是把 Loom 已有能力装配成稳定入口，并把用户带到正确场景。
